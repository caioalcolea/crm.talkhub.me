...
56. **Client-side JWT refresh via invalidateAll**: The client-side `apiRequest()` in `api.js` stores the JWT access token in-memory (`_accessToken`) but NOT the refresh token (security: httpOnly cookie only). When the access token expires (1h), client-side calls get 401. The refresh flow: (1) `refreshAccessToken()` tries `_refreshToken` — returns null. (2) Falls back to `invalidateAll()` which triggers `hooks.server.js` → refreshes token from httpOnly `jwt_refresh` cookie → updates `data.accessToken` → `initClientAuth()` re-runs → `_accessToken` updated. (3) Retries the original request. **Never store the refresh token in JS memory** — always rely on the httpOnly cookie via server-side refresh.
57. **CoworkPiP z-index on non-cowork pages**: All page content containers (`<div class="flex-1">`) MUST have `relative z-20` to stay above the CoworkPiP overlay (`z-index:15`). Without this, the overlay captures mouse events on filter chips, buttons, and other interactive elements. Applied to Tasks, Leads, Cases pages.
58. **Page load performance — lazy form options**: List pages (Tasks, and should be applied to Leads/Cases) MUST NOT fetch dropdown options (users, accounts, contacts, teams, opportunities, cases, leads, tags) in `+page.server.js`. These should be loaded **lazily on client side** when the drawer/form opens, using `clientApiRequest` from `$lib/api.js`. The `+page.server.js` should only fetch the main list data + kanban + pipelines (max 3 API calls). This prevents 8+ unnecessary API calls on every filter click and page navigation. Use `depends('app:<module>')` + `invalidate('app:<module>')` instead of `invalidateAll()` for targeted revalidation.
59. **TaskSerializer inline names**: `TaskSerializer` includes `account_name`, `opportunity_name`, `case_name`, `lead_name` via `SerializerMethodField`. These use `select_related` data already fetched by the view — no extra DB queries. Frontend uses these inline fields to display related entity names without separate API calls.
