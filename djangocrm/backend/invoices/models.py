...
class InvoiceTemplate(OrgScopedMixin, BaseModel):
...
class Invoice(AssignableMixin, OrgScopedMixin, BaseModel):
    """
    ...
    currency = models.CharField(
        _("Currency"), max_length=10, choices=CURRENCY_CODES, default="USD"
    )
...
class InvoiceLineItem(OrgScopedMixin, BaseModel):
...
class Payment(OrgScopedMixin, BaseModel):
...
class Estimate(AssignableMixin, OrgScopedMixin, BaseModel):
...
class EstimateLineItem(OrgScopedMixin, BaseModel):
...
class RecurringInvoice(AssignableMixin, OrgScopedMixin, BaseModel):
...
class RecurringInvoiceLineItem(OrgScopedMixin, BaseModel):
...
class InvoiceHistory(OrgScopedMixin, BaseModel):
    """
