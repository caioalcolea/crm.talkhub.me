...
def send_email(invoice_id, recipients, org_id, domain="localhost", protocol="http"):
    ...
    for user_id in recipients:
        ...
        if profile and profile.user and profile.user.email:
            subject = f"Fatura #{invoice.invoice_number} foi atribuída a você"
            context = {
                "invoice": invoice,
...
def send_invoice_to_client(
    ...
    subject = f"Fatura #{invoice.invoice_number} de {invoice.org.name}"
...
def send_payment_reminder(invoice_id, org_id, domain="localhost", protocol="http"):
    ...
    subject = f"Lembrete de Pagamento: Fatura #{invoice.invoice_number}"
...
def send_estimate_to_client(
    ...
    subject = f"Orçamento #{estimate.estimate_number} de {estimate.org.name}"
