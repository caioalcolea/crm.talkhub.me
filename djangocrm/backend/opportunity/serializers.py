...
class GoalBreakdownSerializer(serializers.ModelSerializer):
    """Read serializer for GoalBreakdown with computed progress fields."""
...
class GoalBreakdownCreateSerializer(serializers.ModelSerializer):
    """Write serializer for GoalBreakdown with sum invariant validation."""
    ...
    def validate_target_value(self, value):
        if value is not None and value <= 0:
            raise serializers.ValidationError(
                "O valor da meta parcial deve ser maior que 0."
            )
        return value

    def validate(self, data):
        """Validate that sum of breakdowns doesn't exceed goal target."""
        ...
        target_value = data.get(
            "target_value",
            getattr(self.instance, "target_value", 0) if self.instance else 0,
        )

        existing_sum = (
            GoalBreakdown.objects.filter(goal=goal)
            .exclude(pk=self.instance.pk if self.instance else None)
            .aggregate(total=Coalesce(Sum("target_value"), Decimal("0")))["total"]
        )

        new_sum = existing_sum + (target_value or 0)
        if new_sum > goal.target_value:
            raise serializers.ValidationError(
                {
                    "target_value": (
                        f"A soma dos desdobramentos ({new_sum}) excede a meta total "
                        f"({goal.target_value}). Disponível: {goal.target_value - existing_sum}."
                    )
                }
            )
        return data
