...
class SalesGoalListView(APIView, LimitOffsetPagination):
    ...
    def get_queryset(self, request):
        ...
        is_admin = request.profile.role == "ADMIN" or request.profile.is_admin
        if not is_admin:
...
class SalesGoalDashboardView(APIView):
    """
    Dashboard endpoint: returns active goals with progress, breakdowns,
    and status in a single call for the goals dashboard page.

    GET /api/opportunities/goals/dashboard/
    Query params:
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = SalesGoal.objects.filter(
            org=org,
            is_active=True,
            period_start__lte=today,
            period_end__gte=today,
        ...
        # Non-admin users only see their own goals or team goals
        is_admin = request.profile.role == "ADMIN" or request.profile.is_admin
        if not is_admin:
            queryset = queryset.filter(
                Q(assigned_to=request.profile)
        ...
        queryset = queryset.select_related(
            "assigned_to", "assigned_to__user", "team"
        ).prefetch_related("breakdowns").distinct()
        ...
        for goal in queryset:
            goal_serialized = SalesGoalSerializer(goal).data

            # Compute days remaining
            days_remaining = max((goal.period_end - today).days, 0)

            # Get breakdowns
            breakdowns = GoalBreakdown.objects.filter(
                goal=goal, org=org
            )
            breakdowns_data = GoalBreakdownSerializer(breakdowns, many=True).data
            ...
            if goal_status in summary:
                summary[goal_status] += 1
