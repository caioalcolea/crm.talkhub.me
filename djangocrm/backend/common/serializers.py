...
class OrgSettingsSerializer(serializers.ModelSerializer):
    ...
    def get_logo_url(self, obj):
        if obj.logo:
            url = obj.logo.url
            # S3/MinIO URLs are already absolute — don't wrap with build_absolute_uri
            # which would prepend the internal Docker host (e.g. http://backend:8000)
            if url.startswith(("http://", "https://", "//")):
                return url
            # Local dev: build full URL from request
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    def get_profile_photo_url(self, obj):
        ...
        # Fallback to User.profile_pic (Google OAuth)
        ...
        return [
            {
                ...
                "has_financial_access": profile.has_financial_access,
            }
...
class ContactFormSubmissionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("name", "email", "subject", "message", "reason", "metadata")

    def validate_email(self, value):
        domain = value.split("@")[-1].lower()
        if domain in disposable_domains:
            raise serializers.ValidationError("Por favor, use um email profissional.")
        return value.lower().strip()

    def validate_reason(self, value):
        valid = {c[0] for c in ContactFormSubmission._meta.get_field("reason").choices}
        if value not in valid:
            raise serializers.ValidationError("Motivo inválido.")
        return value

class PendingInvitationSerializer(serializers.ModelSerializer):
    """Serializer for PendingInvitation model."""
...
class PendingInvitationCreateSerializer(serializers.Serializer):
    ...
    def validate_email(self, value):
        ...
        if domain in disposable_domains:
            raise serializers.ValidationError("E-mails descartáveis não são permitidos.")
        return value
