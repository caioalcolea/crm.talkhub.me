...
class Address(OrgScopedMixin, BaseModel):
...
class Org(BaseModel):
    ...
    default_currency = models.CharField(
        max_length=10, choices=CURRENCY_CODES, default="USD"
    )
...
class Tags(OrgScopedMixin, BaseModel):
    ...
    talkhub_tag_ns = models.CharField(
        max_length=100, blank=True, null=True,
        help_text="TalkHub Omni tag namespace ID",
    )
...
class Profile(OrgScopedMixin, BaseModel):
...
class Comment(OrgScopedMixin, BaseModel):
    """
...
class CommentFiles(OrgScopedMixin, BaseModel):
    """
...
class Attachments(OrgScopedMixin, BaseModel):
    """
...
class Document(OrgScopedMixin, BaseModel):
...
class APISettings(OrgScopedMixin, BaseModel):
...
class Activity(OrgScopedMixin, BaseModel):
...
class Teams(OrgScopedMixin, BaseModel):
...
class PendingInvitation(BaseModel):
    ...
    Quando o email já está registrado, o usuário é vinculado diretamente.
    Quando não está, um convite é criado e um email é enviado.
    """
    ...
    email = models.EmailField(_("Email do convidado"))
    ...
    class Meta:
        verbose_name = "Convite Pendente"
        verbose_name_plural = "Convites Pendentes"
