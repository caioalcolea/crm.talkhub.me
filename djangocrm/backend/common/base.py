...
class OrgScopedMixin(models.Model):
    """
    Mixin para models que JÁ possuem FK 'org' definido manualmente.
    Adiciona OrgScopedManager e validação de org_id no save().

    NÃO define campo org — o model deve definir seu próprio FK/OneToOne.
    Para models NOVOS, use BaseOrgModel em vez deste mixin.

    MRO: class MyModel(AssignableMixin, OrgScopedMixin, BaseModel):
    """
