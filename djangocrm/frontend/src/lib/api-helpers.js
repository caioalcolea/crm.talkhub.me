...
export async function apiRequest(endpoint, options = {}, locals) {
  ...
  // Detect FormData (multipart upload)
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorData.detail) {
        ...
        // Append field_errors if present (validation errors from config_schema)
        if (Array.isArray(errorData.field_errors) && errorData.field_errors.length > 0) {
          errorMessage = errorData.field_errors.join('; ');
        }
