    ...
    >
      {#if tags.length === 0}
        <div class="flex flex-col items-center justify-center py-6 text-center">
          ...
          <p class="text-muted-foreground text-sm font-medium">Nenhuma tag disponível</p>
          <p class="text-muted-foreground/60 mt-1 text-xs">Crie tags para organizar seus leads</p>
        </div>
      {:else}
        ...
        {#if selectedIds.length > 0}
          ...
          >
            <span class="text-primary text-xs font-medium">
              {selectedIds.length} tag{selectedIds.length > 1 ? 's' : ''} selecionada{selectedIds.length > 1 ? 's' : ''}
            </span>
            <button
              type="button"
              onclick={clearAll}
