<script>
  ...
  const displayText = $derived.by(() => {
    ...
    if (startDate) return `De ${formatDateShort(startDate)}`;
      ...
      <div class="flex">
        ...
        <div class="p-3">
          ...
          >
            ...
            {:else}
              <span class="text-muted-foreground text-xs">Data inicial</span>
            {/if}
            ...
            {:else}
              <span class="text-muted-foreground text-xs">
                {selectingEnd ? 'Selecione o fim' : 'Data final'}
              </span>
            {/if}
          </div>
          ...
          >
            {#if selectingEnd}
              <span class="animate-pulse">Clique para selecionar a data final</span>
            {:else}
