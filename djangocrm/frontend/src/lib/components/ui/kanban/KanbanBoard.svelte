<script>
  ...
  // Dynamic board height — fills remaining viewport space regardless of what's above
  ...
  $effect(() => {
    ...
    function setHeight() {
      ...
      el.style.height = `${Math.max(300, window.innerHeight - top - 8)}px`;
    }

    setHeight();

    // Observe parent to catch FilterBar expand/collapse changes
  ...
  async function handleDrop(e, targetColumnId) {
    ...
    try {
      ...
      toast.error(`Falha ao mover ${itemName}`);
...
{#if loading}
  <div class="flex h-96 flex-col items-center justify-center gap-4">
    ...
    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Carregando funil...</p>
  </div>
  ...
  <div class="flex h-96 flex-col items-center justify-center gap-4 text-center">
    ...
    <div>
      ...
      <p class="mt-1 text-sm text-gray-400 dark:text-gray-500">
        Adicione alguns {itemNamePlural} para começar
      </p>
    </div>
  </div>
{:else}
  <!-- Desktop: Horizontal scroll container with all columns -->
  <div class="kanban-board hidden md:flex md:flex-col" bind:this={boardEl}>
    <!-- Pipeline Stats Bar -->
    {#if pipelineStats().totalValue > 0}
      <div
        class="mb-5 shrink-0 flex items-center gap-6 rounded-xl border border-white/10 bg-white/60 px-5 py-3 backdrop-blur-sm dark:border-white/[0.04] dark:bg-white/[0.02]"
      >
        ...
        {#if activeMobileColumn.items.length === 0}
          ...
          >
            ...
            <span class="text-sm font-medium text-gray-400 dark:text-gray-500">
              Nenhum {itemNamePlural} nesta etapa
            </span>
          </div>
        {/if}
  ...
  <!-- Total count footer — mobile only; desktop has counts in column headers -->
  <div class="mt-6 flex items-center justify-center gap-2 text-sm text-gray-400 dark:text-gray-500 md:hidden">
    <div
    ...
    <span class="px-4 font-medium">
      {totalCount}
      {totalCount !== 1 ? itemNamePlural : itemName} em {sortedColumns.length} etapas
    </span>
    <div
      class="h-px flex-1 bg-gradient-to-r from-transparent via-gray-200 to-transparent dark:via-white/10"
    ></div>
