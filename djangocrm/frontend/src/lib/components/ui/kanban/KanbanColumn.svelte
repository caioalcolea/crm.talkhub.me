<script>
  ...
  // Get stage-specific gradient based on column color or name (PT-BR + EN)
  function getColumnGradient(col) {
    const name = col.name?.toLowerCase() || '';
    const color = col.color?.toLowerCase() || '';

    // Closed / Won / Ganho / Fechado
    if (
      name.includes('closed') || name.includes('fechad') ||
      name.includes('won') || name.includes('ganho') || name.includes('vencid') ||
      name.includes('resolv') ||
      color.includes('green')
    ) {
      return 'from-emerald-500 to-teal-500';
    }
    // Lost / Rejected / Perdido / Rejeitado
    if (
      name.includes('lost') || name.includes('perdid') ||
      name.includes('reject') || name.includes('rejeitad') ||
      name.includes('duplic') ||
      color.includes('red')
    ) {
      return 'from-rose-500 to-red-500';
    }
    // Recycled / Nurtured / On Hold / Em Espera
    if (
      name.includes('recycl') || name.includes('recicl') ||
      name.includes('nurtur') || name.includes('espera') ||
      name.includes('hold') ||
      color.includes('purple')
    ) {
      return 'from-violet-500 to-purple-500';
    }
    // In Progress / Qualified / Em Andamento / Qualificado
    if (
      name.includes('process') || name.includes('andamento') ||
      name.includes('qualif') || name.includes('escalad') || name.includes('escalat') ||
      name.includes('aguardand') || name.includes('waiting') || name.includes('pending') ||
      name.includes('pendente') ||
      color.includes('blue')
    ) {
      return 'from-blue-500 to-indigo-500';
    }
    // Assigned / New / Novo / Atribuído
    if (
      name.includes('assign') || name.includes('atribuíd') || name.includes('atribuido') ||
      name.includes('new') || name.includes('novo') || name.includes('nova') ||
      color.includes('cyan')
    ) {
...
<div
  class="kanban-column group flex h-full w-[280px] shrink-0 flex-col rounded-2xl transition-all duration-300"
  ...
  ondrop={onDrop}
  role="region"
  aria-label="{column.name} coluna com {itemCount} {itemCount !== 1 ? itemNamePlural : itemName}"
>
  ...
  <div
    class="column-content flex-1 min-h-0 space-y-2.5 overflow-y-auto border-x border-white/20
