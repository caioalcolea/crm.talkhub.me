<script>
  ...
  const STAGE_STYLES = /** @type {Record<string, { color: string, bgLight: string, text: string }>} */ ({
    PROSPECTING: {
  ...
  const DEFAULT_STYLE = {
    ...
    bgLight: 'bg-[var(--color-primary-default)]/20',
  ...
  const maxValue = $derived(Math.max(...activeStages.map((s) => pipelineData[s.id]?.value || 0), 1));
  const totalPipeline = $derived(
    activeStages.reduce((sum, s) => sum + (pipelineData[s.id]?.value || 0), 0)
  );
</script>
...
>
  ...
  >
    <span class="text-xs font-medium text-[var(--text-secondary)]">Total do Funil Aberto</span>
