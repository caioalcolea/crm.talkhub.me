<script>
  ...
  function formatFollowUp(dateStr) {
    if (!dateStr) return 'Sem retorno agendado';
  ...
  function getLeadName(lead) {
    ...
    return parts.length > 0 ? parts.join(' ') : 'Lead sem nome';
  }
</script>
...
>
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <h3 class="text-sm font-semibold tracking-tight text-[var(--text-primary)]">Leads Quentes</h3>
    </div>
  ...
  <div class="flex-1 overflow-auto">
    {#if leads.length === 0}
      <div class="flex h-full flex-col items-center justify-center py-10 text-center">
        ...
        <p class="text-sm font-medium text-[var(--text-secondary)]">Nenhum lead quente</p>
        <p class="text-xs text-[var(--text-tertiary)]">Marque leads como "Quente" para vê-los aqui</p>
      </div>
    {:else}
