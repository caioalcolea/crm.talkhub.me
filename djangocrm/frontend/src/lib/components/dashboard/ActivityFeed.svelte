<script>
  ...
  const actionConfig =
    /** @type {Record<string, { icon: typeof Activity, bg: string, text: string }>} */ ({
      CREATE: {
        icon: Plus,
  ...
  function formatTime(dateStr) {
    if (!dateStr) return '';
    const date = new Date(dateStr);
    return date.toLocaleTimeString('pt-BR', { hour: 'numeric', minute: '2-digit' });
  }
...
<div
  class="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--surface-raised)] dark:bg-[var(--surface-raised)]/80 dark:backdrop-blur-sm"
>
  ...
  <div class="p-5">
    {#if activities.length === 0}
      <div class="flex flex-col items-center justify-center py-10 text-center">
        ...
        <p class="text-sm font-medium text-[var(--text-secondary)]">Nenhuma atividade recente</p>
        <p class="text-xs text-[var(--text-tertiary)]">As ações aparecerão aqui</p>
      </div>
    {:else}
      <div class="space-y-5">
      ...
      {#if activities.length > 5 && !showAll}
        ...
        >
          Mostrar mais ({activities.length - 5} restantes)
        </Button>
      {/if}
    {/if}
  </div>
