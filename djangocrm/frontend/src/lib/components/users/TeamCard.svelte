<script>
  ...
  function getMemberName(member) {
    return member.user_details?.email?.split('@')[0] || 'Usuário';
  }
...
<Card.Root class="group transition-shadow hover:shadow-md">
    ...
    <div class="flex items-start justify-between">
      ...
      <div class="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <AlertDialog.Root>
          ...
          <AlertDialog.Content>
            <AlertDialog.Header>
              <AlertDialog.Title>Excluir Equipe</AlertDialog.Title>
              <AlertDialog.Description>
                Tem certeza que deseja excluir <strong>{team.name}</strong>? Isso removerá a
                equipe de todos os registros atribuídos. Esta ação não pode ser desfeita.
              </AlertDialog.Description>
            </AlertDialog.Header>
            <AlertDialog.Footer>
              <AlertDialog.Cancel>Cancelar</AlertDialog.Cancel>
              <Button variant="destructive" onclick={() => onDelete?.(team.id)}>Excluir</Button>
            </AlertDialog.Footer>
          </AlertDialog.Content>
        </AlertDialog.Root>
      </div>
  ...
  <Card.Content class="pt-0">
    <div class="flex items-center">
      ...
      {#if members.length === 0}
        <span class="text-muted-foreground text-sm italic">Nenhum membro atribuído</span>
      {/if}
    </div>
  </Card.Content>
</Card.Root>
