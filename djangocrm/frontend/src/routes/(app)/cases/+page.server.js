...
export async function load({ url, locals, cookies }) {
  ...
  try {
    ...
    // Translate kanban column names (backend returns English status values as column names)
    const statusLabels = {
      New: 'Novo',
      Assigned: 'Atribuído',
      Pending: 'Pendente',
    ...
    if (kanbanResponse?.columns) {
      kanbanResponse.columns = kanbanResponse.columns.map((col) => ({
        ...col,
        name: statusLabels[col.name] ?? col.name
      }));
    }
    ...
    throw error(500, `Falha ao carregar casos: ${err.message}`);
  }
}
...
export const actions = {
  create: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!name) {
        return fail(400, { error: 'O nome do caso é obrigatório.' });
      }
  ...
  update: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!name || !caseId) {
        return fail(400, { error: 'Nome e ID do caso são obrigatórios.' });
      }
  ...
  close: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!caseId) {
        return fail(400, { error: 'ID do caso é obrigatório.' });
      }
  ...
  comment: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!body || !caseId) {
        return fail(400, { error: 'Comentário e ID do caso são obrigatórios.' });
      }
