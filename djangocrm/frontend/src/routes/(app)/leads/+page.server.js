...
export async function load({ url, cookies, locals }) {
  ...
  try {
    ...
    throw error(500, `Falha ao carregar leads: ${err.message}`);
  }
}
...
export const actions = {
  create: async ({ request, locals, cookies }) => {
    ...
    try {
      ...
      if (!title) {
        return fail(400, { error: 'O título do lead é obrigatório.' });
      }
  ...
  update: async ({ request, locals, cookies }) => {
    ...
    try {
      ...
      if (!leadId || !title) {
        return fail(400, { error: 'ID e título do lead são obrigatórios.' });
      }
  ...
  convert: async ({ request, locals, cookies }) => {
    try {
      ...
      return fail(400, { error: err.message || 'Falha ao converter' });
    }
  },
  ...
  duplicate: async ({ request, locals, cookies }) => {
    try {
      ...
      if (originalLead.opportunity_amount)
        leadData.opportunity_amount = originalLead.opportunity_amount;
