  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result.type === 'success') {
        toast.success(isCreating ? 'Objetivo criado com sucesso' : 'Objetivo atualizado com sucesso');
        drawerOpen = false;
        await invalidateAll();
      } else if (result.type === 'failure') {
        toast.error(/** @type {string} */ (result.data?.message) || 'Erro ao salvar objetivo');
      } else {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.message) || 'Erro ao excluir objetivo');
      } else {
...
<div class="space-y-6 p-6 md:p-8">
  ...
  <div class="flex items-center gap-2">
    {#each [
      ...
      { key: 'behind', label: 'Atenção Necessária' }
    ] as filter}
      <button
    ...
    <div class="ml-auto w-64">
      <SearchInput bind:value={searchValue} placeholder="Buscar objetivos..." />
    </div>
  </div>
  ...
  {#if filteredGoals.length === 0}
    ...
    >
      ...
      <h3 class="mb-1 text-lg font-semibold text-[var(--text-primary)]">Nenhum objetivo definido</h3>
      <p class="mb-4 text-sm text-[var(--text-secondary)]">
        Crie seu primeiro objetivo de vendas para começar a acompanhar o desempenho.
      </p>
      {#if isAdmin}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-[var(--border-default)] bg-[var(--surface-sunken)]">
            {#each ['Objetivo', 'Tipo', 'Meta', 'Progresso', 'Período', 'Responsável', 'Status'] as label}
              <th
  ...
  {#if data.leaderboard && data.leaderboard.length > 0}
    ...
    >
      <div class="mb-4 flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-semibold text-[var(--text-primary)]">Ranking</h2>
          <p class="text-xs text-[var(--text-tertiary)]">Melhores desempenhos do período atual</p>
        </div>
      </div>
  ...
  <Sheet.Content side="right" class="w-[480px] overflow-hidden border-l p-0 sm:max-w-[480px]">
    <form onsubmit={handleSubmit} class="flex h-full flex-col">
      ...
      <div class="flex items-center justify-between border-b border-[var(--border-default)] px-6 py-4">
        <div class="flex items-center gap-2">
          ...
          {#if isCreating}
            <span class="text-xs text-[var(--text-tertiary)]">Novo</span>
          {/if}
        </div>
        <button
          type="button"
      ...
      <div>
        ...
        >
        <Input id="goal-name" bind:value={formData.name} placeholder="Ex: Meta de Receita Q1" required />
      </div>
      ...
      <div>
        ...
        >
          <Select.Trigger class="w-full">
            {goalTypeOptions.find((o) => o.value === formData.goalType)?.label || 'Selecionar tipo'}
          </Select.Trigger>
          <Select.Content>
      ...
      <div>
        ...
        >
          Meta {formData.goalType === 'REVENUE' ? 'de Valor' : 'de Quantidade'}
        </label>
        <Input
          id="target-value"
          type="number"
      ...
      <div>
        ...
        >
          <Select.Trigger class="w-full">
            {periodTypeOptions.find((o) => o.value === formData.periodType)?.label || 'Selecionar período'}
          </Select.Trigger>
          <Select.Content>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label
            for="period-start"
            class="mb-1.5 block text-xs font-medium text-[var(--text-secondary)]">Data de Início</label
          >
          <Input
            id="period-start"
            type="date"
        ...
        <div>
          <label
            for="period-end"
            class="mb-1.5 block text-xs font-medium text-[var(--text-secondary)]">Data de Término</label
          >
          <Input
            id="period-end"
            type="date"
      ...
      <div>
        ...
        >
          <Select.Trigger class="w-full">
            {userOptions.find((u) => u.id === formData.assignedTo)?.name || 'Selecionar usuário (opcional)'}
          </Select.Trigger>
          <Select.Content>
      ...
      <div>
        ...
        >
          <Select.Trigger class="w-full">
            {teamOptions.find((t) => t.id === formData.team)?.name || 'Selecionar equipe (opcional)'}
          </Select.Trigger>
          <Select.Content>
            <Select.Item value="">Nenhum</Select.Item>
      ...
      {#if !isCreating && selectedGoal}
        ...
        >
          <h3 class="mb-3 text-sm font-semibold text-[var(--text-primary)]">Progresso Atual</h3>
          <div class="space-y-2">
            <div class="flex items-center justify-between text-sm">
              <span class="text-[var(--text-secondary)]">Progresso</span>
            ...
            <div class="flex items-center justify-between text-xs text-[var(--text-tertiary)]">
              <span>
                Alcançado: {selectedGoal.goalType === 'REVENUE'
                  ? formatCurrency(selectedGoal.progressValue, orgCurrency, true)
                  : `${selectedGoal.progressValue} negócios`}
              </span>
              <span>
                Meta: {selectedGoal.goalType === 'REVENUE'
                  ? formatCurrency(selectedGoal.targetValue, orgCurrency, true)
                  : `${selectedGoal.targetValue} negócios`}
              </span>
            </div>
          </div>
        </div>
    ...
    >
      ...
      <div class="flex gap-2">
        ...
        {#if isAdmin}
          <Button type="submit">
            {isCreating ? 'Criar Objetivo' : 'Salvar Alterações'}
          </Button>
        {/if}
      </div>
    </div>
...
<AlertDialog.Root bind:open={deleteDialogOpen}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      <AlertDialog.Title>Excluir Objetivo</AlertDialog.Title>
      <AlertDialog.Description>
        Tem certeza que deseja excluir "{goalToDelete?.name}"? Esta ação não pode ser desfeita.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
