...
export async function load({ locals, url, cookies }) {
  ...
  try {
        ...
        owner:
          account.assigned_to && account.assigned_to.length > 0
            ? {
                ...
                name: account.assigned_to[0].user_details?.email || 'Sem atribuição',
    ...
    throw error(500, `Falha ao carregar contas: ${err.message}`);
  }
}
...
export const actions = {
  create: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!name) {
        return fail(400, { error: 'O nome da empresa é obrigatório.' });
      }
      ...
      return fail(400, { error: err.message || 'Falha ao criar' });
    }
  },
  ...
  update: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!accountId || !name) {
        return fail(400, { error: 'ID e nome da empresa são obrigatórios.' });
      }
      ...
      return fail(400, { error: err.message || 'Falha ao atualizar' });
    }
  },
  ...
  deactivate: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!accountId) {
        return fail(400, { error: 'ID da empresa é obrigatório.' });
      }
      ...
      return fail(400, { error: err.message || 'Falha ao encerrar' });
    }
  },
  ...
  activate: async ({ request, locals, cookies }) => {
    try {
      ...
      return fail(400, { error: err.message || 'Falha ao reabrir' });
    }
  },
  ...
  delete: async ({ request, locals, cookies }) => {
    try {
      ...
      return fail(400, { error: err.message || 'Falha ao excluir' });
    }
  }
};
