<script>
  ...
  // Contact from URL param (for quick action from contact page)
  ...
  const columns = [
    ...
    { key: 'stage', label: 'Etapa', type: 'select', options: stageOptions, width: 'w-32' },
    { key: 'closedOn', label: 'Data de Fechamento', type: 'date', width: 'w-36' },
    {
      key: 'assignedTo',
      label: 'Atribuído a',
      type: 'relation',
  ...
  // Account options for combobox field (id/name format)
  const accountComboOptions = $derived(
    allAccounts.map((a) => ({
      id: a.id,
      name: a.name
    }))
  ...
  const allAccounts = $derived([...formOptions.accounts, ...localAccounts]);
  ...
  async function handleCreateAccount(name) {
    try {
      ...
      if (newAccountId) {
        ...
        drawerFormData = { ...drawerFormData, account: newAccountId };
        toast.success(`Empresa "${name}" criada com sucesso`);
      }
    } catch (err) {
      toast.error(`Erro ao criar empresa: ${/** @type {Error} */ (err).message || 'Erro desconhecido'}`);
    }
  }
  ...
  const stageFilterOptions = $derived([{ value: '', label: 'Todas as Etapas' }, ...stageOptions]);
  ...
  // URL sync for accountId/contactId and action params (quick action from account/contact page)
  $effect(() => {
    ...
    if (action === 'create' && !drawerOpen) {
      ...
      // Handle contact pre-fill from URL
      if (contactIdParam) {
      ...
      // Set contact in form data after drawer opens
      if (contactIdParam) {
        drawerFormData.contacts = [contactIdParam];
      }
    }
  });
  ...
  async function saveFieldValue(rowId, field, value, fieldType) {
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao salvar alterações');
      }
    } catch (err) {
  ...
  function openDrawer(rowId) {
    selectedRowId = rowId;
    drawerMode = 'view';
    // Initialize form data from the selected row
    const row = filteredOpportunities.find((/** @type {any} */ r) => r.id === rowId);
    if (row) {
      drawerFormData = {
        ...row,
        account: row.account?.id || '',
        currency: row.currency || $orgSettings.default_currency || 'BRL',
        contacts: (row.contacts || []).map((/** @type {any} */ c) => c.id),
        assignedTo: (row.assignedTo || []).map((/** @type {any} */ u) => u.id),
  ...
  function closeDrawer() {
    drawerOpen = false;
    selectedRowId = null;
    // Always clear URL params on close (no-op if no params present)
    clearUrlParams();
  }
  ...
  async function handleDrawerUpdate() {
    ...
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao atualizar negócio');
      }
    } catch (err) {
  ...
  async function handleDrawerSave() {
    ...
    try {
      const form = new FormData();
      form.append('name', drawerFormData.name || 'Novo Negócio');
      form.append('stage', drawerFormData.stage || 'PROSPECTING');
      form.append('amount', drawerFormData.amount?.toString() || '0');
      form.append('probability', drawerFormData.probability?.toString() || '50');
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao criar negócio');
      }
    } catch (err) {
  ...
  async function deleteSelectedRow() {
    ...
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao excluir negócio');
      }
    } catch (err) {
  ...
  async function addLineItem() {
    ...
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao adicionar produto');
      }
    } catch (err) {
  ...
  async function deleteLineItem(lineItemId) {
    ...
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao remover produto');
      }
    } catch (err) {
  ...
  async function confirmCreateInvoice() {
    ...
    try {
      ...
      if (result.type === 'success' || result.data?.success) {
        ...
        toast.error(result.data?.message || 'Falha ao criar fatura');
      }
    } catch (err) {
...
<svelte:head>
  <title>Negócios - CRMTalkHub</title>
</svelte:head>
...
<CrmDrawer
  ...
  titleKey="name"
  titlePlaceholder="Nome do negócio"
  headerLabel="Negócio"
  mode={drawerMode}
  loading={isLoading}
...
>
  {#snippet headerContent()}
    ...
    {:else if selectedRow?.contacts?.length > 0}
      <div class="space-y-1">
        <label class="text-xs font-medium text-muted-foreground">Contato Principal</label>
        <a
          href="/contacts?view={selectedRow.contacts[0].id}"
    ...
    {#if drawerMode !== 'create' && selectedRow}
      <div class="mb-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p class="text-xs text-[var(--text-tertiary)]">Empresa</p>
          ...
          {#if !isClosed && selectedRow.daysInStage != null}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">Dias na Etapa</p>
          ...
          {#if isClosed && selectedRow.closedBy}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">Fechado por</p>
              <p class="font-medium text-[var(--text-primary)]">
          ...
          {#if isClosed && selectedRow.closedOn}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">Fechado em</p>
              <p class="font-medium text-[var(--text-primary)]">
      ...
      <div class="mt-6 border-t border-[var(--border-default)] pt-4">
              ...
              >
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-xs text-[var(--text-secondary)]">
                    ...
                    = {formatCurrency(item.total, selectedRow.currency || 'BRL')}
                  </p>
                </div>
                {#if !isClosed}
                  <button
        ...
        {:else}
          <p class="mb-4 text-sm text-[var(--text-secondary)]">Nenhum produto adicionado.</p>
        {/if}
        ...
        {#if !isClosed}
          ...
          >
            <p class="text-xs font-medium text-[var(--text-secondary)]">Adicionar Produto</p>
              ...
              >
                ...
                <Select.Content class="max-h-60">
                  ...
                  {#if filteredProductOptions.length === 0}
                    <div class="px-3 py-2 text-sm text-gray-500">Nenhum produto encontrado</div>
                  {:else}
    ...
    {:else}
      ...
      {#if canCreateInvoice}
        <AlertDialog.Root bind:open={createInvoiceDialogOpen}>
          ...
          <AlertDialog.Content>
            <AlertDialog.Header>
              <AlertDialog.Title>Criar Fatura</AlertDialog.Title>
              <AlertDialog.Description>
                Isso criará uma nova fatura a partir desta oportunidade com {lineItems.length} produto{lineItems.length ===
                1
                  ? ''
                  : 's'} totalizando {formatCurrency(
                  selectedRow?.lineItemsTotal || 0,
                  selectedRow?.currency || 'BRL'
                )}.
              </AlertDialog.Description>
            ...
            <AlertDialog.Footer>
              ...
              <AlertDialog.Action onclick={confirmCreateInvoice}>Criar Fatura</AlertDialog.Action>
            </AlertDialog.Footer>
          </AlertDialog.Content>
        </AlertDialog.Root>
      {/if}
