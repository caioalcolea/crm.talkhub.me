...
export async function load({ url, locals, cookies }) {
  ...
  try {
      ...
      owner:
        contact.assigned_to && contact.assigned_to.length > 0
          ? {
              ...
              name: contact.assigned_to[0].user_details?.email || 'Sem atribuição',
    ...
    throw error(500, `Falha ao carregar contatos: ${err.message}`);
  }
}
...
export const actions = {
  create: async ({ request, locals, cookies }) => {
    try {
      ...
      const tiktok = form.get('tiktok')?.toString().trim() || '';
      const telegram = form.get('telegram')?.toString().trim() || '';
      // Address fields
  ...
  delete: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!contactId) {
        return fail(400, { error: 'ID do contato é obrigatório' });
      }
