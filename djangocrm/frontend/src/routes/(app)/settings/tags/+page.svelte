  ...
  $effect(() => {
    if (form?.success) {
      const messages = {
        ...
        update: 'Etiqueta atualizada com sucesso',
        archive: 'Etiqueta arquivada com sucesso',
        restore: 'Etiqueta restaurada com sucesso'
      };
      toast.success(messages[form.action] || 'Operação realizada com sucesso');
      closeDialogs();
      invalidateAll();
    } else if (form?.error) {
...
<svelte:head>
  <title>Etiquetas - TalkHub CRM</title>
</svelte:head>
...
<div class="border-border bg-card border-b">
  <div class="px-6 py-5">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-foreground text-2xl font-semibold tracking-tight">Etiquetas</h1>
        <p class="text-muted-foreground mt-1 text-sm">
          Crie e gerencie etiquetas para organizar contatos, empresas, negócios e chamados.
        </p>
      </div>
      <Button
        onclick={openCreate}
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors {showArchived
            ...
            : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'}"
        >
          Arquivadas ({archivedTags.length})
        </button>
      </div>
    </div>
  </div>
  ...
  <div class="px-6 py-4">
    {#if !showArchived}
      {#if filteredActiveTags.length === 0}
        <div class="flex flex-col items-center justify-center py-16 text-center">
          ...
          {#if searchQuery}
            <h3 class="text-foreground text-lg font-medium">Nenhuma etiqueta encontrada</h3>
            <p class="text-muted-foreground mt-1 text-sm">
              Nenhuma etiqueta corresponde a "{searchQuery}". Tente um termo de busca diferente.
            </p>
          {:else}
            <h3 class="text-foreground text-lg font-medium">Nenhuma etiqueta ainda</h3>
            <p class="text-muted-foreground mt-1 max-w-sm text-sm">
              Etiquetas ajudam você a organizar e filtrar seus registros do CRM. Crie sua primeira etiqueta para começar.
            </p>
            <Button
              onclick={openCreate}
      ...
      {:else}
        ...
        <p class="text-muted-foreground mt-3 text-xs">
          Exibindo {filteredActiveTags.length} de {activeTags.length} etiquetas ativas
        </p>
      {/if}
    {:else if filteredArchivedTags.length === 0}
      <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        {#if searchQuery}
          <h3 class="text-foreground text-lg font-medium">Nenhuma etiqueta arquivada encontrada</h3>
          <p class="text-muted-foreground mt-1 text-sm">
            Nenhuma etiqueta arquivada corresponde a "{searchQuery}".
          </p>
        {:else}
          <h3 class="text-foreground text-lg font-medium">Nenhuma etiqueta arquivada</h3>
          <p class="text-muted-foreground mt-1 text-sm">Etiquetas arquivadas aparecerão aqui.</p>
        {/if}
      </div>
    {:else}
      <div class="border-border overflow-hidden rounded-lg border">
      ...
      <p class="text-muted-foreground mt-3 text-xs">
        Exibindo {filteredArchivedTags.length} de {archivedTags.length} etiquetas arquivadas
      </p>
    {/if}
  </div>
</div>
...
<Dialog.Root bind:open={createDialogOpen}>
  <Dialog.Content class="gap-0 overflow-hidden p-0 sm:max-w-md">
    <div class="border-border border-b px-6 py-4">
      <Dialog.Title class="text-lg font-semibold">Criar uma etiqueta</Dialog.Title>
      <Dialog.Description class="text-muted-foreground mt-1 text-sm">
        Etiquetas ajudam você a organizar e segmentar seus registros.
      </Dialog.Description>
    </div>
    <form
      method="POST"
      ...
      <div class="space-y-5 px-6 py-5">
        <div class="space-y-2">
          <Label for="name" class="text-sm font-medium">
            Nome da etiqueta <span class="text-destructive">*</span>
          </Label>
          <Input
            id="name"
            name="name"
        ...
        <!-- Preview -->
        <div class="pt-2">
          <Label class="text-muted-foreground text-sm font-medium">Pré-visualização</Label>
...
<Dialog.Root bind:open={editDialogOpen}>
  <Dialog.Content class="gap-0 overflow-hidden p-0 sm:max-w-md">
    <div class="border-border border-b px-6 py-4">
      <Dialog.Title class="text-lg font-semibold">Editar etiqueta</Dialog.Title>
      <Dialog.Description class="text-muted-foreground mt-1 text-sm">
        Atualize o nome, cor ou descrição da etiqueta.
      </Dialog.Description>
    </div>
    <form
      method="POST"
