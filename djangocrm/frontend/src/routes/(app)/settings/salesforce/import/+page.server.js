...
export const actions = {
  startImport: async ({ request, cookies }) => {
    ...
    if (!objectTypesRaw) {
      return fail(400, { importError: 'Selecione pelo menos um tipo de objeto.' });
    }
    ...
    try {
      ...
      return fail(400, { importError: err?.message || 'Falha na operação' });
    }
  },
  cancelImport: async ({ request, cookies }) => {
    ...
    if (!jobId) {
      return fail(400, { importError: 'ID do job não fornecido.' });
    }

    if (!UUID_RE.test(jobId)) {
      return fail(400, { importError: 'ID do job inválido.' });
    }
