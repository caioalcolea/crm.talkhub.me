<script>
  ...
  const OBJECT_TYPES = [
    { value: 'Account', label: 'Contas', description: 'Empresas e organizações', depends: [] },
    { value: 'Contact', label: 'Contatos', description: 'Pessoas associadas às contas', depends: ['Account'] },
    { value: 'Opportunity', label: 'Oportunidades', description: 'Negócios e pipeline de vendas', depends: ['Account'] },
    { value: 'Product2', label: 'Produtos', description: 'Catálogo de produtos', depends: [] },
    { value: 'Order', label: 'Pedidos', description: 'Pedidos de venda', depends: ['Account'] },
    { value: 'Quote', label: 'Cotações', description: 'Cotações e orçamentos', depends: ['Account', 'Contact'] }
  ];
  ...
  $effect(() => {
    ...
    if (form?.importCancelled) toast.success('Importação cancelada');
  });
...
<svelte:head>
  <title>Importação Salesforce - TalkHub CRM</title>
</svelte:head>

<PageHeader title="Importar do Salesforce" subtitle="Selecione os dados para importar">
  {#snippet actions()}
...
<div class="mx-auto max-w-4xl space-y-6 p-6 md:p-8">
    ...
    <Card.Root class="border-blue-200 dark:border-blue-800">
      <Card.Header>
        <div class="flex items-center gap-3">
          ...
          <div>
            <Card.Title>Importação em Andamento</Card.Title>
            <Card.Description>Seus dados do Salesforce estão sendo importados...</Card.Description>
          </div>
        </div>
      </Card.Header>
      <Card.Content>
        <div class="space-y-4">
          <div>
            <div class="mb-2 flex items-center justify-between text-sm">
              <span class="text-muted-foreground">Progresso Geral</span>
                ...
                <div class="bg-muted/50 rounded-lg p-3">
                  ...
                  {#if processed > 0 || total > 0}
                    ...
                    <div class="mt-1 flex gap-4 text-xs">
                      <span class="text-emerald-600 dark:text-emerald-400">{imported} importados</span>
                      <span class="text-amber-600 dark:text-amber-400">{skipped} ignorados</span>
                      <span class="text-red-600 dark:text-red-400">{errors} erros</span>
                    </div>
                  {/if}
                </div>
              {/each}
          ...
          <form method="POST" action="?/cancelImport" use:enhance={() => {
            ...
            <span class="text-muted-foreground ml-2 text-xs">Os dados já importados serão mantidos.</span>
          </form>
        </div>
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root class={activeJob.status === 'COMPLETED' ? 'border-emerald-200 dark:border-emerald-800' : activeJob.status === 'CANCELLED' ? 'border-amber-200 dark:border-amber-800' : 'border-red-200 dark:border-red-800'}>
      <Card.Header>
        <div class="flex items-center gap-3">
          ...
          {/if}
          <div>
            <Card.Title>Importação {activeJob.status === 'COMPLETED' ? 'Concluída' : activeJob.status === 'CANCELLED' ? 'Cancelada' : 'Falhou'}</Card.Title>
            <Card.Description>
              {#if activeJob.status === 'COMPLETED'}
                Seus dados do Salesforce foram importados com sucesso.
              {:else if activeJob.status === 'CANCELLED'}
                A importação foi cancelada. Os dados já importados foram mantidos.
              {:else}
                A importação encontrou erros. Verifique os detalhes abaixo.
              {/if}
            </Card.Description>
          </div>
        </div>
      ...
      <Card.Content>
        <div class="flex items-center justify-between text-sm">
          ...
          <div class="flex gap-4 text-xs">
            <span class="text-emerald-600 dark:text-emerald-400">{activeJob.imported_count || 0} importados</span>
            <span class="text-amber-600 dark:text-amber-400">{activeJob.skipped_count || 0} ignorados</span>
            <span class="text-red-600 dark:text-red-400">{activeJob.error_count || 0} erros</span>
          </div>
        </div>
      </Card.Content>
    </Card.Root>
  ...
  {#if !isImportActive}
    <Card.Root>
      <Card.Header>
        <Card.Title>Selecionar Dados para Importar</Card.Title>
        <Card.Description>Escolha quais objetos do Salesforce você deseja importar. As dependências serão resolvidas automaticamente.</Card.Description>
      </Card.Header>
      <Card.Content>
        <div class="space-y-3">
            ...
            >
              ...
              <div class="flex-1">
                ...
                {#if objType.depends.length > 0}
                  <div class="text-muted-foreground/70 mt-1 text-xs">Depende de: {objType.depends.join(', ')}</div>
                {/if}
              </div>
            </button>
          {/each}
        ...
        <div class="mt-6 flex items-center justify-between">
          <p class="text-muted-foreground text-sm">
            {selectedTypes.size} objeto{selectedTypes.size !== 1 ? 's' : ''} selecionado{selectedTypes.size !== 1 ? 's' : ''}
          </p>
  ...
  {#if importHistory.length > 0}
    <Card.Root>
      <Card.Header>
        <Card.Title>Histórico de Importação</Card.Title>
        <Card.Description>Clique em uma linha para ver detalhes</Card.Description>
      </Card.Header>
        ...
        <Table.Root>
          <Table.Header>
            <Table.Row>
              ...
              <Table.Head>Objetos</Table.Head>
              <Table.Head class="text-right">Importados</Table.Head>
              <Table.Head class="text-right">Ignorados</Table.Head>
              <Table.Head class="text-right">Erros</Table.Head>
            </Table.Row>
          </Table.Header>
          <Table.Body>
            {#each importHistory as job}
              ...
              <Table.Row class="cursor-pointer hover:bg-muted/50" onclick={() => toggleJobExpand(job.id)}>
                ...
                <Table.Cell class="text-muted-foreground text-sm">
                  {job.created_at ? formatRelativeDate(job.created_at) : 'Desconhecido'}
                </Table.Cell>
                      ...
                      {#if skips.length > 0}
                        <div>
                          <h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400">
                            {skips.length} {skips.length === 1 ? 'Registro' : 'Registros'} Ignorados
                          </h4>
