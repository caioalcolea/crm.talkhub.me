<script>
  ...
  const drawerFields = [
    ...
    { key: 'primaryColor', label: 'Cor Primária', type: 'color', section: 'branding' },
    { key: 'secondaryColor', label: 'Cor Secundária', type: 'color', section: 'branding' },
    { key: 'defaultNotes', label: 'Notas Padrão', type: 'textarea', section: 'defaults' },
    { key: 'defaultTerms', label: 'Termos Padrão', type: 'textarea', section: 'defaults' },
    { key: 'footerText', label: 'Texto do Rodapé', type: 'text', section: 'defaults' },
    { key: 'isDefault', label: 'Definir como Padrão', type: 'boolean', section: 'settings' }
  ];
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao criar modelo');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao atualizar modelo');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao excluir modelo');
      }
    };
  }}
>
...
<div class="flex flex-col gap-4 p-6">
  <!-- Header -->
  <PageHeader title="Modelos de Fatura">
    {#snippet actions()}
      <!-- Back to Invoices -->
  ...
  <CrmTable data={templates} {columns} bind:visibleColumns onRowClick={handleRowClick}>
    {#snippet emptyState()}
      <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 class="text-lg font-medium text-[var(--text-primary)]">Nenhum modelo ainda</h3>
        <p class="text-sm text-[var(--text-secondary)]">Crie seu primeiro modelo de fatura</p>
      </div>
    {/snippet}
...
<CrmDrawer
  ...
  titleKey="name"
  titlePlaceholder="Novo Modelo"
  headerLabel="Modelo de Fatura"
  mode={drawerMode}
  ...
  {#snippet footerActions()}
    <div class="flex w-full items-center justify-end gap-2">
      ...
      <Button onclick={handleDrawerSave}>
        {drawerMode === 'create' ? 'Criar Modelo' : 'Salvar Alterações'}
      </Button>
    </div>
  {/snippet}
  ...
  {#snippet activitySection()}
    {#if selectedTemplate}
      <div class="border-t pt-4">
        <h4 class="text-muted-foreground mb-3 text-sm font-medium">Pré-visualização</h4>
        <div
