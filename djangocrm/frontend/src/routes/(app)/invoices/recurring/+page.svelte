<script>
  ...
  const drawerFields = [
    // Core Info Section
    { key: 'title', label: 'Título', type: 'text', section: 'core', required: true },
    {
      key: 'isActive',
      label: 'Ativo',
      type: 'boolean',
    ...
    { key: 'clientEmail', label: 'E-mail do Cliente', type: 'email', section: 'client' },
    // Schedule Section
    {
      key: 'frequency',
      label: 'Frequência',
    ...
    { key: 'customDays', label: 'Dias Personalizados', type: 'number', section: 'schedule' },
    { key: 'startDate', label: 'Data de Início', type: 'date', section: 'schedule' },
    { key: 'endDate', label: 'Data de Término', type: 'date', section: 'schedule' },
    { key: 'nextGenerationDate', label: 'Data da Próxima Fatura', type: 'date', section: 'schedule' },
    // Settings Section
    {
      key: 'paymentTerms',
  ...
  async function handleDelete() {
    ...
    if (!confirm('Tem certeza que deseja excluir esta fatura recorrente?')) return;
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        toast.success('Fatura recorrente criado(a)');
        closeDrawer();
        invalidateAll();
      } else if (result.type === 'failure') {
        toast.error(
          /** @type {string} */ (result.data?.error) || 'Falha ao criar fatura recorrente'
        );
      }
    };
  }}
...
<form
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        toast.success('Fatura recorrente atualizado(a)');
        closeDrawer();
        invalidateAll();
      } else if (result.type === 'failure') {
        toast.error(
          /** @type {string} */ (result.data?.error) || 'Falha ao atualizar fatura recorrente'
        );
      }
    };
  }}
...
<form
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        toast.success('Fatura recorrente excluído(a)');
        closeDrawer();
        invalidateAll();
      } else if (result.type === 'failure') {
        toast.error(
          /** @type {string} */ (result.data?.error) || 'Falha ao excluir fatura recorrente'
        );
      }
    };
  }}
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao atualizar status');
      }
    };
  }}
>
...
<div class="flex flex-col gap-4 p-6">
  <!-- Header -->
  <PageHeader title="Faturas Recorrentes">
    {#snippet actions()}
      <!-- Back to Invoices -->
  ...
  <CrmTable data={recurringInvoices} {columns} bind:visibleColumns onRowClick={handleRowClick}>
    {#snippet emptyState()}
      <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 class="text-lg font-medium text-[var(--text-primary)]">Nenhuma fatura recorrente ainda</h3>
        <p class="text-sm text-[var(--text-secondary)]">
          Crie seu primeiro modelo de fatura recorrente
        </p>
      </div>
    {/snippet}
...
<CrmDrawer
  ...
  titleKey="title"
  titlePlaceholder="Nova Fatura Recorrente"
  headerLabel="Fatura Recorrente"
  mode={drawerMode}
    ...
    <div class="flex w-full items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <Button onclick={handleDrawerSave}>
          {drawerMode === 'create' ? 'Criar Recorrente' : 'Salvar Alterações'}
        </Button>
      </div>
    </div>
  {/snippet}
  ...
  {#snippet activitySection()}
    {#if selectedRecurring}
      <div class="border-t pt-4">
        <h4 class="text-muted-foreground mb-2 text-sm font-medium">Estatísticas</h4>
        <div class="space-y-2 text-sm">
          ...
          {#if selectedRecurring.endDate}
            <div class="flex justify-between">
              <span class="text-muted-foreground">Data de Término</span>
              <span>{formatDate(selectedRecurring.endDate)}</span>
            </div>
          {/if}
          <div class="flex justify-between">
            <span class="text-muted-foreground">Próxima Fatura</span>
            <span
          ...
          <div class="flex justify-between">
            <span class="text-muted-foreground">Faturas Geradas</span>
