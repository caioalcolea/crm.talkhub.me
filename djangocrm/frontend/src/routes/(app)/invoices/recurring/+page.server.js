...
export async function load({ url, locals, cookies }) {
  ...
  try {
    ...
    throw error(500, `Falha ao carregar faturas recorrentes: ${err.message}`);
  }
}
...
export const actions = {
  ...
  update: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!recurringId) {
        return fail(400, { error: 'ID da fatura recorrente é obrigatório' });
      }
  ...
  toggle: async ({ request, locals, cookies }) => {
    try {
      ...
      return { success: true, message: 'Status alternado com sucesso' };
    } catch (err) {
