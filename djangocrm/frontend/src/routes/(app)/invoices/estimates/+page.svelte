<script>
  ...
  const drawerFields = [
    // Core Info Section
    { key: 'estimateNumber', label: 'Orçamento #', type: 'readonly', section: 'core' },
    { key: 'title', label: 'Título', type: 'text', section: 'core' },
    {
      key: 'status',
      label: 'Status',
      type: 'select',
    ...
    // Client Section
    { key: 'clientName', label: 'Nome do Cliente', type: 'text', section: 'client', required: true },
    { key: 'clientEmail', label: 'Email do Cliente', type: 'email', section: 'client' },
    { key: 'clientPhone', label: 'Telefone do Cliente', type: 'text', section: 'client' },
    // Dates Section
    { key: 'issueDate', label: 'Data de Emissão', type: 'date', section: 'dates' },
    { key: 'expiryDate', label: 'Válido até', type: 'date', section: 'dates' },
    ...
    // Notes Section
    { key: 'notes', label: 'Notas', type: 'textarea', section: 'notes' },
    { key: 'terms', label: 'Termos e Condições', type: 'textarea', section: 'notes' }
  ];
  ...
  const STATUS_CHIPS = [
    { key: 'ALL', label: 'Todos' },
    { key: 'OPEN', label: 'Aberto', statuses: ['Draft', 'Sent', 'Viewed'] },
    { key: 'ACCEPTED', label: 'Aceito', statuses: ['Accepted'] },
    { key: 'DECLINED', label: 'Recusado', statuses: ['Declined'] },
    { key: 'EXPIRED', label: 'Expirado', statuses: ['Expired'] }
  ];
  ...
  function openCreateDrawer() {
    ...
    drawerMode = 'create';
    drawerFormData = {
      estimateNumber: 'Gerado automaticamente',
      title: '',
      status: 'Draft',
      clientName: '',
      clientEmail: '',
  ...
  async function handleConvert() {
    ...
    if (!confirm('Converter este orçamento em uma fatura?')) return;
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao criar orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao atualizar orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao excluir orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao enviar orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao converter orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao aceitar orçamento');
      }
    };
  }}
>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao recusar orçamento');
      }
    };
  }}
>
  ...
  <CrmTable data={estimates} {columns} bind:visibleColumns onRowClick={handleRowClick}>
    {#snippet emptyState()}
      <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 class="text-lg font-medium text-[var(--text-primary)]">Nenhum orçamento ainda</h3>
...
<CrmDrawer
  ...
  titlePlaceholder="Novo Orçamento"
    ...
    <div class="flex w-full items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <Button onclick={handleDrawerSave}>
          {drawerMode === 'create' ? 'Criar Orçamento' : 'Salvar Alterações'}
        </Button>
      </div>
    </div>
  {/snippet}
  ...
  {#snippet activitySection()}
    {#if selectedEstimate}
      <div class="border-t pt-4">
        <h4 class="text-muted-foreground mb-2 text-sm font-medium">Atividade</h4>
