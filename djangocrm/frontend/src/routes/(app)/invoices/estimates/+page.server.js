...
export async function load({ url, locals, cookies }) {
  ...
  try {
    ...
    const transformedEstimates = estimates.map((estimate) => ({
      ...
      // Currency
      currency: estimate.currency || 'BRL',
      // Notes
    ...
    throw error(500, `Falha ao carregar orçamentos: ${err.message}`);
  }
}
...
export const actions = {
  ...
  update: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!estimateId) {
        return fail(400, { error: 'ID do orçamento é obrigatório' });
      }
  ...
  convert: async ({ request, locals, cookies }) => {
    try {
      ...
      return {
        success: true,
        message: 'Orçamento convertido em fatura',
  ...
  accept: async ({ request, locals, cookies }) => {
    try {
      ...
      return { success: true, message: 'Orçamento marcado como aceito' };
    } catch (err) {
  ...
  decline: async ({ request, locals, cookies }) => {
    try {
      ...
      return { success: true, message: 'Orçamento marcado como recusado' };
    } catch (err) {
