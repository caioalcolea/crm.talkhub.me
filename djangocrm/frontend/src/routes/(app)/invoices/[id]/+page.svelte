...
<svelte:head>
  <title>Fatura {invoice.invoiceNumber} | TalkHub CRM</title>
</svelte:head>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      if (result.type === 'redirect') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao cancelar fatura');
      }
    };
  }}
></form>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      ...
      if (result.type === 'success') {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao enviar');
      }
    };
  }}
></form>
  ...
  use:enhance={() => {
    return async ({ result }) => {
      ...
      if (result.type === 'success' && result.data?.pdf) {
        ...
        toast.error(/** @type {string} */ (result.data?.error) || 'Falha ao baixar PDF');
      }
    };
  }}
></form>
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <div class="flex items-center gap-3">
            <h1 class="text-xl font-semibold">{invoice.invoiceNumber || 'Fatura'}</h1>
            <Select.Root type="single" value={invoice.status} onValueChange={handleStatusChange}>
              ...
              >
                {INVOICE_STATUS_OPTIONS.find((o) => o.value === (invoice.status || 'Draft'))?.label || (invoice.status || 'Rascunho').replace('_', ' ')}
              </Select.Trigger>
              <Select.Content>
          ...
          <p class="text-muted-foreground text-sm">{invoice.clientName || 'Sem nome do cliente'}</p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        {#if isEditing}
        ...
        {:else}
          ...
          {#if invoice.status !== 'Cancelled' && invoice.status !== 'Paid'}
            <AlertDialog.Root bind:open={cancelDialogOpen}>
              ...
              <AlertDialog.Content>
                <AlertDialog.Header>
                  <AlertDialog.Title>Cancelar Fatura?</AlertDialog.Title>
                  <AlertDialog.Description>
                    Isso irá cancelar a fatura {invoice.invoiceNumber}. Faturas canceladas não podem ser
                    enviadas ou editadas.
                  </AlertDialog.Description>
                </AlertDialog.Header>
                <AlertDialog.Footer>
                  <AlertDialog.Cancel>Manter Fatura</AlertDialog.Cancel>
                  <Button
                    variant="destructive"
                    onclick={() => {
      ...
      <div class="bg-background rounded-lg border shadow-sm print:hidden">
            ...
            <div class="space-y-3 text-right">
              ...
              {:else}
                <div class="flex items-center justify-end gap-2 text-sm">
                  <span class="text-muted-foreground">Data de Emissão:</span>
                ...
                <div class="flex items-center justify-end gap-2 text-sm">
                  <span class="text-muted-foreground">Moeda:</span>
        ...
        <div class="grid grid-cols-2 gap-8 border-b p-6">
          ...
          <div class="space-y-3">
            ...
            {:else}
              <p class="font-medium">
                {accounts.find((a) => a.id === invoice.accountId)?.name || 'Sem conta'}
              </p>
            {/if}
          </div>
          ...
          <div class="space-y-3">
            ...
            {:else}
              <div class="space-y-1">
                <p class="font-medium">{invoice.clientName || 'Sem nome'}</p>
