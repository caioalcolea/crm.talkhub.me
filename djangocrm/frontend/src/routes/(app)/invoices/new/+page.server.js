...
export async function load({ locals, cookies }) {
  ...
  try {
    ...
    throw error(500, `Falha ao carregar dados do formulário: ${err.message}`);
  }
}
...
export const actions = {
  create: async ({ request, locals, cookies }) => {
    try {
      ...
      if (!accountId) {
        return fail(400, { error: 'Empresa é obrigatória' });
      }
      if (!contactId) {
        return fail(400, { error: 'Contato é obrigatório' });
      }
      ...
      if (!clientEmail) {
        return fail(400, { error: 'E-mail do cliente é obrigatório' });
      }
