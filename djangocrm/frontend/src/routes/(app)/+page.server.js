...
export async function load({ locals, cookies }) {
  ...
  try {
    ...
    // Translations for activity feed
    const actionTranslations = /** @type {Record<string, string>} */ ({
      'Created': 'Criou', 'Updated': 'Atualizou', 'Deleted': 'Excluiu',
      'Viewed': 'Visualizou', 'Commented': 'Comentou', 'Assigned': 'Atribuiu'
    });
    const entityTranslations = /** @type {Record<string, string>} */ ({
      'Lead': 'Lead', 'Contact': 'Contato', 'Account': 'Conta',
      'Opportunity': 'Oportunidade', 'Case': 'Chamado', 'Task': 'Tarefa',
      'Invoice': 'Fatura', 'Goal': 'Objetivo', 'Team': 'Equipe',
      'User': 'Usuário', 'Estimate': 'Orçamento', 'Product': 'Produto'
    });

    // Transform activities from dashboard response
    const recentActivities = (dashboardResponse.activities || []).map((activity) => {
      const actionPt = actionTranslations[activity.action_display] || activity.action_display;
      const entityPt = entityTranslations[activity.entity_type] || activity.entity_type;
      return {
        id: activity.id,
        user: {
          name: activity.user?.email?.split('@')[0] || 'N/A'
        },
        ...
        description:
          ...
          `${actionPt} ${entityPt}: ${activity.entity_name}`,
