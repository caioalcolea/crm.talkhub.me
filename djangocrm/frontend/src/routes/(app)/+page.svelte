<script>
  ...
  const currencyNote = $derived(
    otherCurrencyCount > 0
      ? `Apenas ${orgCurrency} (${otherCurrencyCount} em outras moedas)`
      : `Apenas ${orgCurrency}`
  );
...
<svelte:head>
  <title>Painel - TalkHub CRM</title>
</svelte:head>
      ...
      <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        <div class="space-y-2">
          ...
          <p class="max-w-md text-sm text-[var(--text-secondary)]">
            Veja o que está acontecendo no seu CRM hoje. Acompanhe seu funil e feche mais
            negócios.
          </p>
        </div>
      </div>
