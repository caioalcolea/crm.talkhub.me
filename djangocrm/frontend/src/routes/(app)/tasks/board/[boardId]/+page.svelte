<script>
  ...
  let newColumnColor = $state('#6B7280');

  // Track which column is showing the add task form
  let addingTaskToColumn = $state(null);
...
<div class="board-container">
  <header class="board-header">
    <a href="/tasks" class="back-link">Voltar aos Quadros</a>
              ...
              {#if task.assignedTo && task.assignedTo.length > 0}
                <div class="task-assignees">
                  {#each task.assignedTo.slice(0, 3) as assignee}
                    <span class="assignee-avatar" title={assignee.name || assignee.user_details?.email}>
                      {(assignee.name || assignee.user_details?.email || '?').charAt(0).toUpperCase()}
                    </span>
                  {/each}
          ...
          {#if addingTaskToColumn === column.id}
            ...
            >
              ...
              <div class="form-actions">
                ...
                <button type="button" class="btn-secondary" onclick={cancelAddingTask}
                  >Cancelar</button
                >
              </div>
            </form>
          {:else}
            <button class="add-task-btn" onclick={() => startAddingTask(column.id)}>
              + Adicionar Tarefa
            </button>
          {/if}
        </div>
      ...
      {#if showAddColumn}
        ...
        >
          ...
          <div class="form-actions">
            ...
            <button type="button" class="btn-secondary" onclick={() => (showAddColumn = false)}
              >Cancelar</button
            >
          </div>
        </form>
