...
export async function load({ params, locals, cookies }) {
  ...
  try {
    ...
    throw error(500, err.message || 'Falha ao carregar quadro');
  }
}
...
export const actions = {
  ...
  createColumn: async ({ params, request, locals, cookies }) => {
    ...
    try {
      ...
      if (!name) {
        return fail(400, { error: 'Nome da coluna é obrigatório' });
      }
  ...
  createTask: async ({ params, request, locals, cookies }) => {
    ...
    try {
      ...
      if (!columnId || !title) {
        return fail(400, { error: 'ID da coluna e título são obrigatórios' });
      }
  ...
  updateTask: async ({ request, locals, cookies }) => {
    ...
    try {
      ...
      if (!taskId) {
        return fail(400, { error: 'ID da tarefa é obrigatório' });
      }
  ...
  moveTask: async ({ request, locals, cookies }) => {
    ...
    try {
      ...
      if (!taskId || !columnId) {
        return fail(400, { error: 'ID da tarefa e ID da coluna são obrigatórios' });
      }
