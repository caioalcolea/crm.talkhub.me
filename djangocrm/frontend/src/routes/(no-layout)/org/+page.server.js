...
export async function load({ cookies, locals }) {
  ...
  // Check for pending invitation org auto-selection
  const inviteOrgId = cookies.get('invite_org_id');
  if (inviteOrgId) {
    cookies.delete('invite_org_id', { path: '/' });
  }

  if (!user) {
    return { orgs: [], inviteOrgId: inviteOrgId || null };
  }
...
export const actions = {
  selectOrg: async ({ request, cookies }) => {
    ...
    if (!orgId || !UUID_RE.test(orgId)) {
      return fail(400, { error: 'ID da organização inválido' });
    }
    ...
    try {
      ...
      return fail(500, { error: 'Falha ao trocar de organização' });
    }
  }
};
