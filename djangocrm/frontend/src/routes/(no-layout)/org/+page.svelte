<script>
  ...
  // Auto-select invited org
  let inviteFormEl = $state(null);
  $effect(() => {
    if (data.inviteOrgId && inviteFormEl) {
      loading = true;
      selectedOrgId = data.inviteOrgId;
...
<svelte:head>
  <title>Selecionar Organização | TalkHub CRM</title>
</svelte:head>
  ...
  <main class="flex flex-1 items-start justify-center px-6 py-12">
    <div class="w-full max-w-2xl">
      ...
      <div class="mb-8 text-center">
        <h1 class="text-2xl font-bold text-[var(--text-primary)]">Selecione uma organização</h1>
        <p class="mt-2 text-[var(--text-secondary)]">
          Escolha em qual organização você deseja trabalhar
        </p>
      </div>
      ...
      {:else}
        ...
        >
          ...
          <h3 class="text-lg font-semibold text-[var(--text-primary)]">Nenhuma organização ainda</h3>
          <p class="mt-2 text-[var(--text-secondary)]">
            Crie sua primeira organização para começar a usar o TalkHub CRM
          </p>
          <a
            href="/org/new"
      ...
      {#if data.inviteOrgId}
        <form
          ...
          class="hidden"
        >
          <input type="hidden" name="org_id" value={data.inviteOrgId} />
