...
<svelte:head>
  <title>Fatura #{invoice.invoiceNumber} | {invoice.org?.name || 'Fatura'}</title>
</svelte:head>
...
>
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold" style="color: {primaryColor};">
            {invoice.org?.name || 'Fatura'}
          </h1>
          <p class="text-gray-500">Fatura #{invoice.invoiceNumber}</p>
        </div>
        <button
          onclick={downloadPDF}
  ...
  <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="overflow-hidden rounded-xl bg-white shadow-lg">
      ...
      <div class="border-b bg-gray-50 p-6 sm:p-8">
        <div class="flex flex-col justify-between gap-6 sm:flex-row">
          <div>
            <h2 class="text-lg font-semibold text-gray-900">Cobrar de:</h2>
      ...
      {#if invoice.payments && invoice.payments.length > 0}
        <div class="border-t bg-green-50 p-6 sm:p-8">
          <h3 class="mb-4 text-lg font-medium text-gray-900">Histórico de Pagamentos</h3>
          <div class="space-y-3">
    ...
    <div class="mt-8 text-center text-sm text-gray-500">
      ...
      <p class="mt-2">Se tiver alguma dúvida sobre esta fatura, entre em contato conosco.</p>
    </div>
  </div>
</div>
