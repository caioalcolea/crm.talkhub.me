...
export async function load({ params, fetch }) {
  ...
  if (!token) {
    throw error(400, 'Token da fatura é obrigatório');
  }
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        throw error(404, 'Fatura não encontrada ou link expirado');
      }
      throw error(response.status, 'Falha ao carregar fatura');
    }
    ...
    throw error(500, 'Falha ao carregar fatura');
  }
}
