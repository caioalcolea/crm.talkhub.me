...
<svelte:head>
  <title>Orçamento #{estimate.estimateNumber} | {estimate.org?.name || 'Orçamento'}</title>
</svelte:head>
...
>
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold" style="color: {primaryColor};">
            {estimate.org?.name || 'Orçamento'}
          </h1>
          <p class="text-gray-500">Orçamento #{estimate.estimateNumber}</p>
        </div>
        <button
          onclick={downloadPDF}
      ...
      <div class="rounded-lg border border-green-200 bg-green-50 p-4">
        <p class="text-green-800">
          {#if form.action === 'accepted'}
            Obrigado! Você aceitou este orçamento. Entraremos em contato em breve.
          {:else if form.action === 'declined'}
            Você recusou este orçamento. Obrigado pela sua resposta.
          {/if}
        </p>
      </div>
    </div>
  ...
  <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="overflow-hidden rounded-xl bg-white shadow-lg">
      ...
      <div class="border-b bg-gray-50 p-6 sm:p-8">
        <div class="flex flex-col justify-between gap-6 sm:flex-row">
          <div>
            <h2 class="text-lg font-semibold text-gray-900">Preparado Para:</h2>
          ...
          <div class="text-left sm:text-right">
            <div class="flex flex-col gap-2">
              ...
              <div class="text-sm text-gray-600">
                <span class="font-medium">Data de Emissão:</span>
                {formatDate(estimate.issueDate)}
              </div>
              {#if estimate.expiryDate}
                <div class="text-sm text-gray-600">
                  <span class="font-medium">Válido Até:</span>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b text-left text-sm text-gray-500">
                ...
                <th class="pb-3 text-right font-medium">Preço Unitário</th>
        ...
        <div class="mt-6 border-t pt-6">
          <div class="flex flex-col items-end gap-2">
            ...
            {#if Number(estimate.taxAmount) > 0}
              <div class="flex w-full justify-between text-sm sm:w-64">
                <span class="text-gray-500">Imposto:</span>
                <span class="text-gray-900"
      ...
      {#if canRespond()}
        <div class="border-t bg-blue-50 p-6 sm:p-8">
          <h3 class="mb-4 text-lg font-medium text-gray-900">Sua Resposta</h3>
          <p class="mb-6 text-gray-600">
            Por favor, revise este orçamento e nos informe se deseja prosseguir.
          </p>
      ...
      {:else if estimate.status === 'Accepted'}
        <div class="border-t bg-green-50 p-6 sm:p-8">
          <div class="flex items-center gap-3">
            ...
            <p class="font-medium text-green-800">Você aceitou este orçamento. Obrigado!</p>
          </div>
        </div>
      {:else if estimate.status === 'Declined'}
        <div class="border-t bg-gray-100 p-6 sm:p-8">
          <p class="text-gray-600">Este orçamento foi recusado.</p>
        </div>
      {:else if isExpired()}
        <div class="border-t bg-orange-50 p-6 sm:p-8">
          <p class="text-orange-800">
            Este orçamento expirou. Entre em contato conosco se desejar uma cotação atualizada.
          </p>
        </div>
      {/if}
      ...
      {#if estimate.notes || estimate.terms}
        <div class="border-t bg-gray-50 p-6 sm:p-8">
          {#if estimate.notes}
            <div class="mb-4">
              <h4 class="mb-2 text-sm font-medium text-gray-700">Observações</h4>
              <p class="text-sm whitespace-pre-wrap text-gray-600">{estimate.notes}</p>
            </div>
          {/if}
          {#if estimate.terms}
            <div>
              <h4 class="mb-2 text-sm font-medium text-gray-700">Termos e Condições</h4>
    ...
    <div class="mt-8 text-center text-sm text-gray-500">
      ...
      {/if}
      <p>Obrigado por considerar nossos serviços!</p>
      <p class="mt-2">Se tiver alguma dúvida sobre este orçamento, entre em contato conosco.</p>
    </div>
  </div>
</div>
